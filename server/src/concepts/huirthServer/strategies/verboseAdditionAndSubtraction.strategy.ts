...
export const huirthServerVerboseAdditionAndSubtractionStrategyTopic =
  (length: number): string => 'Starting from zero add or subtract ' + huirth_convertNumberToStringVerbose(length) + ' random numbers from a total sum, and then output the final sum';
export const huirthServerVerboseAdditionAndSubtractionStrategy = (length: number): ActionStrategy => {
  ...
  for (let i = 0; i < length; i++) {
    ...
    actions.push( even ? huirthServerInnerAddTo({
      addTo: number
